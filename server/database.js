    ...
    try {
        if (json.response.length < 1) {
            ...
            writableStream.write(
                'id' + delim + 'time_stamp' + delim + 'reg_number' + delim + 'altitude' + delim +
                'direction' + delim + 'speed' + delim + 'latitude' + delim + 'longitude' + delim +
                'dep_iata' + delim + 'flight_icao' + delim + 'status' + `\n`, 'UTF8'
            );

            for (let i = 0; i < json.response.length; i++) {
                const f = json.response[i];
                writableStream.write(
                    currentID + delim + getDateTime() + delim + f.reg_number + delim +
                    f.alt + delim + f.dir + delim + f.speed + delim +
                    f.lat + delim + f.lng + delim + f.dep_iata + delim +
                    f.flight_icao + delim + f.status + `\n`, 'UTF8'
                );
                currentID++;
            }
...
const loadCSV2 = async () => {
    ...
    let csvStream = fastcsv
        ...
        .on("end", function () {
          ...
          (id, time_stamp, reg_number, altitude, direction, speed, latitude, longitude, dep_iata, flight_icao, status) 
          ...
          ON CONFLICT (id) DO NOTHING;`;
            ...
            pool.connect((err, client, done) => {
                ...
                try {
                    csvData.forEach(row => {
                        const [id, ts, reg, alt, dir, speed, lat, lng, dep, icao, status] = row;

                        // Skip rows with missing numeric values
                        if (
                            [alt, dir, speed, lat, lng].includes("undefined") ||
                            [alt, dir, speed, lat, lng].some(val => val === undefined || val === "")
                        ) {
                            console.warn("Skipping row due to undefined numeric values:", row);
                            return;
                        }
...
// Wrapper to call both CSV writer and database loader
